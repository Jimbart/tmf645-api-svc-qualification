...
describe('Videotron (Email) Case', () => {
  test('Should send email to Videotron if LPDS_ID found expire or NoFootPrint in Firestore', async () => {
    const sendEmailToVideotron = async () => {
      ...
      const parsedResult = await queryPartnerDB('TPSP10');
      ...
      return new Promise((resolve, reject) => {
        const newAmsMap =
          '3606 Avenue de Port-Royal, Mock Address City Videotron ';
        //const province = 'QC';
    ...
    const sendEmailResponse = await sendEmailToVideotron();
    ...
    // Temporary Email fot Development purposes
