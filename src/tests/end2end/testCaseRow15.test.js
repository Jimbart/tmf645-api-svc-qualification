...
describe('COGECO Mock Email Test using MailSlurp', () => {
  test('Should send email to COGECO if LPDS_ID found expire or NoFootPrint in Firestore', async () => {
    const sendEmailToCogeco = async () => {
      ...
      const parsedResult = await queryPartnerDB('TPSP04');
      ...
      return new Promise((resolve, reject) => {
        const newAmsMap = '123 Baywalk st, Cogeco Mock Address City Ontario CA';
        //const province = 'ON';
    ...
    const sendEmailResponse = await sendEmailToCogeco();
    ...
    expect(sendEmailResponse.to.toString()).toMatch(
      'jameshenry.ching@telusinternational.com'
    );
  });
});
